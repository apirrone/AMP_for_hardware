...
class ObservationBuffer:
    def __init__(self, num_envs, num_obs, include_history_steps, device):
        ...
        self.obs_buf = torch.zeros(
            self.num_envs, self.num_obs_total, device=self.device, dtype=torch.float
        )
    ...
    def get_lagged_obs(self, indices):
        ...
        for i, env_id in enumerate(self.obs_buf):
            idx = indices[i]
            obs.append(self.obs_buf[i, idx * self.num_obs])
            if len(self.obs_buf[i, idx * self.num_obs]) != 51:
                print("AAAAA")
                print(len(self.obs_buf[i, idx * self.num_obs]))
        print("===")
